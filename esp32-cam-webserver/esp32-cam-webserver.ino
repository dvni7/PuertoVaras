...
// Primary config, or defaults.
    ...
    stationList[] = {{"microscopio_05","123456789", true}};
#endif
...
#elif defined(DEFAULT_INDEX_PROJECT)
    char default_index[] = "project";
#else
...
// Robot control telemetry (extends default camera server with project-specific controls)
float robotPosX = 0.0f;
float robotPosY = 0.0f;
int robotHeading = 0;          // Heading in degrees (0-359)
bool robotInMotion = false;
String robotMotionState = "idle";
int cameraTiltDegrees = 0;     // Camera tilt servo angle
unsigned long photoCaptureCount = 0;
...
void handleRobotMoveCommand(const char *direction) {
    ...
    robotInMotion = true;
    robotMotionState = direction;
    if (strcmp(direction, "forward") == 0) {
        robotPosY += robotMoveStep;
...
void nudgeCameraTilt(int delta) {
    setCameraTilt(cameraTiltDegrees + delta);
}

void tiltCameraUp() {
    nudgeCameraTilt(cameraTiltStep);
}
...
String firebaseMicrosToString(uint64_t value) {
    char buffer[24];
    snprintf(buffer, sizeof(buffer), "%llu", static_cast<unsigned long long>(value));
    return String(buffer);
}

String sanitizeFirebaseIdentifier(const String &value) {
    String sanitized;
...
String firebaseNetworkLabel() {
    if (accesspoint) {
        #if defined(FIREBASE_AP_LABEL)
            return sanitizeFirebaseIdentifier(String(FIREBASE_AP_LABEL));
        #else
            if (strlen(apName) > 0) {
                return sanitizeFirebaseIdentifier(String(apName));
            }
            if (stationCount > 0) {
                return sanitizeFirebaseIdentifier(String(stationList[0].ssid));
            }
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        if (ssid.length() > 0) {
            return sanitizeFirebaseIdentifier(ssid);
        }
    }
    return sanitizeFirebaseIdentifier(String(myName));
}
...
String firebaseTimestamp() {
    ...
    return firebaseMicrosToString(microsNow);
}
...
bool ensureFirebaseReady() {
    ...
    if (millis() - lastFirebaseAttempt < firebaseRetryMs) {
        return false;
    }
    ...
    String email = "";
    String password = "";
    #if defined(FIREBASE_USER_EMAIL)
        email = FIREBASE_USER_EMAIL;
    #endif
    #if defined(FIREBASE_USER_PASSWORD)
        password = FIREBASE_USER_PASSWORD;
    #endif

    Serial.println("Initialising Firebase client");
    if (!Firebase.signUp(&firebaseConfig, &firebaseAuth, email.c_str(), password.c_str())) {
        Serial.printf("Firebase signUp failed: %s\r\n", firebaseConfig.signer.signupError.message.c_str());
        return false;
    }

    Firebase.begin(&firebaseConfig, &firebaseAuth);
    firebaseDB.setResponseSize(8 * 1024);
