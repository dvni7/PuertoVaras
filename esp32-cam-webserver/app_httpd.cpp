...
extern int cameraTiltMinDegrees;
extern int cameraTiltMaxDegrees;
extern int cameraTiltStep;
...
extern int cameraTiltDegrees;
extern unsigned long photoCaptureCount;
...
extern void setCameraTilt(int newTilt);
extern void tiltCameraUp();
extern void tiltCameraDown();
...
void serialDump() {
   ...
   Serial.printf("MCU temperature : %i C, %i F  (approximate)\r\n", McuTc, McuTf);
    Serial.printf("Robot Position: X=%.1f, Y=%.1f, Heading=%i deg\r\n", robotPosX, robotPosY, robotHeading);
    Serial.printf("Robot Motion: %s (active: %s)\r\n", robotMotionState.c_str(), robotInMotion ? "yes" : "no");
    Serial.printf("Camera Tilt: %i deg, Photos captured: %lu\r\n", cameraTiltDegrees, photoCaptureCount);
...
static esp_err_t capture_handler(httpd_req_t *req){
    ...
    if(fb->format == PIXFORMAT_JPEG){
        ...
        handleCapturedPhoto(fb->buf, fb_len);
    } else {
        res = ESP_FAIL;
...
static esp_err_t cmd_handler(httpd_req_t *req){
    ...
    if(!strcmp(variable, "framesize")) {
        ...
        if (strcmp(value, "stop") == 0) {
            ...
            handleRobotRotateCommand("left");
        } else if (strcmp(value, "rotate_right") == 0) {
            handleRobotRotateCommand("right");
        } else {
    ...
    else if(!strcmp(variable, "camera_set_tilt")) {
        setCameraTilt(val);
    }
...
static esp_err_t status_handler(httpd_req_t *req){
    ...
    *p++ = '{';
    ...
    if (critERR.length() == 0) {
        ...
        p+=sprintf(p, "\"camera_tilt\":%d,", cameraTiltDegrees);
        p+=sprintf(p, "\"camera_tilt_min\":%d,", cameraTiltMinDegrees);
        p+=sprintf(p, "\"camera_tilt_max\":%d,", cameraTiltMaxDegrees);
        p+=sprintf(p, "\"camera_tilt_step\":%d,", cameraTiltStep);
        p+=sprintf(p, "\"robot_x\":%.2f,", robotPosX);
        p+=sprintf(p, "\"robot_y\":%.2f,", robotPosY);
        p+=sprintf(p, "\"robot_heading\":%d,", robotHeading);
        p+=sprintf(p, "\"robot_motion\":\"%s\",", robotMotionState.c_str());
        p+=sprintf(p, "\"robot_active\":%u,", robotInMotion);
        p+=sprintf(p, "\"photos_taken\":%lu,", photoCaptureCount);
...
static esp_err_t index_handler(httpd_req_t *req){
    ...
    } else if(strncmp(view,"project", sizeof(view)) == 0) {
        Serial.println("Project interface requested");
        if (critERR.length() > 0) return error_handler(req);
        httpd_resp_set_type(req, "text/html");
        httpd_resp_set_hdr(req, "Content-Encoding", "identity");
        return httpd_resp_send(req, (const char *)index_project_html, index_project_html_len);
